...
BlockExporterTools.prototype.getGeneratorHTML =
  ...
  for (var blockType in blockXmlMap) {
    ...
    if (xml) {
      // Render the preview block in the hidden workspace.
      var tempBlock = FactoryUtils.getDefinedBlock(blockType, this.hiddenWorkspace);
      // Get generator stub for the given block and add to  generator code.
      var blockGenDoc = FactoryUtils.getDoc(tempBlock);
    } else {
      // Append warning comment and write to console.
      var blockGenDoc = '// No generator stub generated for ' + blockType +
        '. Block was not found in Block Library Storage.';
      console.log('No block generator stub generated for ' + blockType +
        '. Block was not found in Block Library Storage.');
    }
    multiblockDoc.push(blockGenDoc);
  }
  return multiblockDoc;
};
