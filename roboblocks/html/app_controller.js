...
AppController.prototype.formatBlockLibraryForExport_ = function(blockXmlMap) {
  ...
  for (var blockType in blockXmlMap) {
	  //console.log(blockXmlMap[blockType]);
	  var blockXmlDom = (new DOMParser).parseFromString(blockXmlMap[blockType], "text/xml").childNodes[0];
	  var instruction = goog.dom.createElement('block_library_item');
	Array.from(blockXmlDom.children).forEach(function(item) {
	 instruction.appendChild(item);
    });
	instruction.setAttribute('name',instruction.childNodes[0].childNodes[3].childNodes[0].data);
	xmlDom.appendChild(instruction);
  }
...
AppController.prototype.formatBlockLibraryForImport_ = function(xmlText) {
  //console.log((new DOMParser).parseFromString(xmlText, "text/xml").childNodes[0].childNodes);
  var xmlDom = (new DOMParser).parseFromString(xmlText, "text/xml").childNodes[0];
  //console.log(xmlDom);
  ...
  //console.log(xmlDom.childNodes);
  //var dummy = (new DOMParser).parseFromString(xmlText, "text/xml");
  
  //var blockNodes = (new DOMParser).parseFromString(xmlText, "text/xml").childNodes[0].childNodes;
  var blockNodes = xmlDom.childNodes;
  ...
  Array.from(blockNodes).forEach(function(blockNode) {
	  var _xmlDom = goog.dom.createDom('xml', {
      'xmlns':"http://www.w3.org/1999/xhtml"
		});
	  Array.from(blockNode.childNodes).forEach(function(item){
		  _xmlDom.appendChild(item);
		  //console.log(Blockly.Xml.domToText(item));
	  });
	  //console.log(Blockly.Xml.domToText(_xmlDom));
	  blockXmlTextMap[blockNode.attributes[0].value] = Blockly.Xml.domToText(_xmlDom);
    });
 ...
 *    AppController.EXAMPLE_FACTORY, or AppController.EXPORTER
 * @private
 */
...
/*AppController.prototype.onTab = function() {
  ...
  var exampleFactoryTab = this.tabMap[AppController.EXAMPLE_FACTORY];
  ...
  // selected.
  this.exampleFactoryController.keyEventsEnabled =
      this.selectedTab == AppController.EXAMPLE_FACTORY;
  ...
  if (this.selectedTab == AppController.EXPORTER) {
    ...
    // factory.
    this.exampleFactoryController.saveStateFromWorkspace();

    // Update exporter's list of the types of blocks used in workspace factory.
    var usedBlockTypes = this.exampleFactoryController.getAllUsedBlockTypes();
  ...
  } else if (this.selectedTab == AppController.EXAMPLE_FACTORY) {
    ...
    FactoryUtils.show('exampleFactoryContent');
    ...
    this.exampleFactoryController.setBlockLibCategory(categoryXml,
        blockTypes);
  }
...
AppController.prototype.addBlockFactoryEventListeners = function() {
  ...
  BlockFactory.mainWorkspace.addChangeListener(BlockFactory.undoRedo);
...
AppController.prototype.onresize = function(event) {
	...
	if (preview!==null)
	{
		...
		{
		  bbox = BlockFactory.previewWorkspace.getAllBlocks()[0].getHeightWidth();
		  preview.style.height = (bbox.height+20)+ 'px';
		  //preview.style.width = (wrap.offsetWidth/3 - 40) + 'px';
		  if (wrap === undefined || wrap=== null)
			  height = 115;
		  else
			  height = (wrap.offsetHeight-bbox.height-2)/3;
		}
		else
		{
    ...
    //var height = (wrap.offsetHeight-(bbox.height+20));
	//console.log(height);
	//console.log((wrap.offsetHeight/4 - 2));
	...
	//blockly.style.width = (2*wrap.offsetWidth/3 - 2) + 'px';
	if (wrap === undefined || wrap=== null)
	{
		blockly.style.position = "relative";
		if (languageDiv!==null)
		  languageDiv.style.height = height+ 'px';
		//languagePre.style.width = (wrap.offsetWidth/3 - 2) + 'px';
		if (languagePre!==null)
		  languagePre.style.height = height + 'px';
		//generatorDiv.style.width = (wrap.offsetWidth/3 - 2) + 'px';
		if (generatorDiv!==null)
		  generatorDiv.style.height = height + 'px';
		//generatorPre.style.width = (wrap.offsetWidth/3 - 2) + 'px';
		if (generatorPre!==null)
		  generatorPre.style.height = height + 'px';
		docDiv.style.height = ((blockly.parentNode.offsetHeight-bbox.height-2)-2*height) + 'px';
		docDiv.style.position = 'relative';
	}
	else{
		blockly.style.height = (wrap.offsetHeight - 2) + 'px';
		if (languageDiv!==null)
		  languageDiv.style.height = height+ 'px';
		//languagePre.style.width = (wrap.offsetWidth/3 - 2) + 'px';
		if (languagePre!==null)
		...
		if (generatorPre!==null)
		  generatorPre.style.height = height + 'px';
		//docDiv.style.width = (wrap.offsetWidth/3 - 2) + 'px';
		if (docDiv!==null)
		docDiv.style.height = height + 'px';
	}
	
	//languagePre.style.width = (wrap.offsetWidth/3 - 2) + 'px';
    //languagePre.style.height = (wrap.offsetHeight/4 - 2) + 'px';
    /*var expandList = [
      document.getElementById('blocklyPreviewContainer'),
      document.getElementById('blockly'),
      //document.getElementById('blocklyMask'),
      //document.getElementById('previewDoc'),
      //document.getElementById('languagePre'),
      //document.getElementById('languageTA'),
      //document.getElementById('generatorPre'),
	  //document.getElementById('docDiv'),
    ];
...
/*AppController.prototype.confirmLeavePage = function(e) {
  ...
  if ((!BlockFactory.isStarterBlock() &&
      ...
      blocklyFactory.exampleFactoryController.hasUnsavedChanges()) {
...
AppController.prototype.mainWorkspace = function() {
	return BlockFactory.mainWorkspace;
};
...
AppController.prototype.init = function() {
 ...
 // if ('BlocklyStorage' in window && window.location.hash.length > 1) {
  ...
  //ExampleFactoryInit.initWorkspaceFactory(this.exampleFactoryController);
};
